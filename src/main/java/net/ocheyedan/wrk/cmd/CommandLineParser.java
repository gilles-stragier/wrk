...
public final class CommandLineParser {
     ...
     */
    public static Command parse(String[] args, RestTemplate restTemplate) {
        if ((args == null) || (args.length < 1)) {
            return new Cards(parse(args, 0), restTemplate);
        } else if ("--usage".equals(args[0]) || "--help".equals(args[0]) || "help".equals(args[0])) {
            return new Usage(parse(args, 1), restTemplate);
        } else if ("--version".equals(args[0]) || "-v".equals(args[0]) || "-version".equals(args[0])) {
            return new Version(parse(args, 1), restTemplate);
        } else if ("orgs".equals(args[0])) {
            return new Orgs(parse(args, 1), restTemplate);
        } else if ("boards".equals(args[0])) {
            return new Boards(parse(args, 1), restTemplate);
        } else if ("lists".equals(args[0])) {
            return new Lists(parse(args, 1), restTemplate);
        } else if ("cards".equals(args[0])) {
            return new Cards(parse(args, 1), restTemplate);
        } else if ("comments".equals(args[0])) {
            return new Comments(parse(args, 1), restTemplate);
        } else if ("members".equals(args[0])) {
            return new Members(parse(args, 1), restTemplate);
        } else if ("desc".equals(args[0])) {
            return new Desc(parse(args, 1), restTemplate);
        } else if ("search".equals(args[0])) {
            return new Search(parse(args, 1), restTemplate);
        } else if ("comment".equals(args[0])) {
            return new Comment(parse(args, 1), restTemplate);
        } else if ("assign".equals(args[0])) {
            return new Assign(parse(args, 1), restTemplate);
        } else if ("unassign".equals(args[0])) {
            return new UnAssign(parse(args, 1), restTemplate);
        } else if ("create".equals(args[0])) {
            return new Create(parse(args, 1), restTemplate);
        } else if ("move".equals(args[0])) {
            return new Move(parse(args, 1), restTemplate);
        } else if ("label".equals(args[0])) {
            return new Labels(parse(args, 1), restTemplate);
        } else if ("unlabel".equals(args[0])) {
            return new UnLabels(parse(args, 1), restTemplate);
        } else if ("close".equals(args[0])) {
            return new Close(parse(args, 1), restTemplate);
        } else if ("pop".equals(args[0])) {
            return new Pop(parse(args, 1), restTemplate);
        } else {
            return new Usage(parse(args, 0), restTemplate);
        }
    }
